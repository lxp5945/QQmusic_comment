...
def getComments(information):#获取评论数据
    ...
    for page in range(2,5):#前4页评论
        ...
        for req1_data in req1:
            ...
            with open('QQ音乐评论.csv','a',encoding='utf-8',newline='') as f:
                write=csv.writer(f)
                write.writerow([sing_name,star_name,attention,publishTime,user,otherStyleTime,comment])

                # f.write(sing_name,star_name,attention,publishTime,user,otherStyleTime,comment)
            a=a+1
            if a>=25:
                print(seqno)
...
def threed():
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as e:
        ...
        for li in lis:
            information_list_tuple = resolve(li)
