...
def get_proxy():
    ...
    proxy_json = requests.get(
        url='http://tiqu.pyhttp.taolop.com/getip?count=2&neek=15872&type=2&yys=0&port=2&sb=&mr=2&sep=0&ts=1&ys=1&cs=1').json()
    # print('获取到的代理:', proxy_json)
    proxy = proxy_json['data'][0]['ip'] + ":" + str(proxy_json['data'][0]['port'])
    proxy = ['110.86.174.62:64256', '110.81.106.193:64256']
...
def Obtain(url):  # 获取html
    urls = []
    res = requests.get(url, headers=headers).text  # proxies=get_proxy()
...
def resolve(url):  # 获取数据
    for href in Obtain(url):
        data_res = requests.get(href, headers=headers).text
        ...
        address = data.css('.crumbs.fl>h1>a::text').get().replace('二手房成交', '')
        ...
        print(title, TransactionHour, price, unitPrice, houseType, transactionOwnership, constructionArea,
        ...
        # sheet.append([title,TransactionHour,price,unitPrice,houseType,transactionOwnership,constructionArea,houseStructure,innerArea,buildingType,yearOfCompletion,renovationCondition,buildingStructure,heatingMethod,theProportionOfTerracedHouseholds,houseOrientation,equippedWithElevator])
        with open('京东.csv', 'a', encoding='utf-8', newline='') as f:
            write = csv.writer(f)
            write.writerow(
...
# {'南通经济技术开发区','启东市','如东县','如皋市','崇川区','海安市','海门区','港闸区','通州区'}
