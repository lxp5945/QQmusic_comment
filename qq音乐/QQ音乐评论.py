    ...
    for information in information_list_tuple[0:2]:#歌曲数量
        ...
        for page in range(2,5):#前4页评论
            ...
            for req1_data in req1:
                ...
                # # fileheader = ['sing_name','star_name','attention','user','otherStyleTime','comment']
                # csvFile = open("instance.csv", "w", encoding='utf-8-sig', newline='')
                # fileheader = ['sing_name', 'star_name', 'attention', 'user', 'otherStyleTime', 'comment']
                # dict_writer = csv.DictWriter(csvFile, fileheader)

                with open('QQ音乐评论hh.csv', 'a', encoding='utf-8-sig', newline='') as f:
                    write = csv.writer(f)
                    write.writerow([sing_name, star_name,attention,user, otherStyleTime,comment])
                print(sing_name,star_name,attention,user,otherStyleTime,comment)
                a=a+1
                if a>=25:
                    print(seqno)
